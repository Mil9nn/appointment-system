...
const Page = () => {
  return (
    <section className="relative min-h-screen overflow-hidden">
      {/* Background Accent */}
      <div className="absolute top-0 left-0 w-[750px] h-full bg-blue-500/20 blur-3xl"></div>
      <div className="absolute bottom-0 right-0 w-[750px] h-full bg-pink-500/20 blur-3xl"></div>

      {/* Content Wrapper */}
      <div className="relative max-w-7xl mx-auto px-6 py-5 space-y-20">
        <div className="flex items-center justify-between">
          <h3 className="text-3xl font-semibold tracking-wide uppercase text-blue-400">Harmony Care</h3>
          <p className="flex items-center gap-3">
            <Link href="/auth/signup" className="self-start font-semibold hover:text-blue-500 transition-colors duration-1000 ease-in-out cursor-pointer">Signup</Link>
            <span className="inline-block text-2xl font-extrabold rounded-full w-0.5 h-8 bg-white -skew-12"></span>
            <Link href="/auth/login" className="self-end font-semibold hover:text-blue-500 transition-colors duration-1000 ease-in-out cursor-pointer">Login</Link>
          </p>
        </div>
        ...
        <div className="text-center space-y-6">
          ...
          <div>
            <button className="relative group z-0 overflow-hidden px-8 py-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-full shadow-lg transition-all duration-300 cursor-pointer">
              ...
              <span className="absolute top-0 left-0 translate-y-full z-5 group-hover:translate-y-0 transition-transform duration-1000 ease-in-out w-full h-full bg-black"></span>
            </button>
          </div>
        </div>
